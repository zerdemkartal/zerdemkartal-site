...
const field = { border: '1px solid #E8E3D6', borderRadius: 10, padding: '13px 15px', fontSize: 15, fontFamily: 'inherit', width: '100%', boxSizing: 'border-box' };
...
export default function UyeAuth() {
  ...
  if (member) {
    return (
      <div style={{ background: '#FFFFFF', border: '1px solid #E8E3D6', borderRadius: 22, padding: '34px 34px 38px' }}>
        <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#DCE7DB', display: 'grid', placeItems: 'center', fontFamily: "'Newsreader', serif", fontSize: 24 }}>
          {(member.name || '?').slice(0, 1).toUpperCase()}
        </div>
        <div style={{ fontFamily: "'Newsreader', serif", fontSize: 26, marginTop: 16 }}>Merhaba, {member.name}</div>
        <div style={{ color: '#6B675E', fontSize: 14, marginTop: 4 }}>{member.email}</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 24 }}>
          <a href="/astroloji-101" style={{ border: '1px solid #E8E3D6', borderRadius: 12, padding: '13px 16px', color: '#2B1D12', textDecoration: 'none', fontSize: 15 }}>✦ Astroloji 101 — kaldığın yerden</a>
          <a href="/dogum-haritasi" style={{ border: '1px solid #E8E3D6', borderRadius: 12, padding: '13px 16px', color: '#2B1D12', textDecoration: 'none', fontSize: 15 }}>☾ Doğum haritanı çıkar</a>
        </div>
        <button onClick={signout} style={{ marginTop: 22, background: 'none', border: 'none', color: '#6B675E', fontSize: 14, cursor: 'pointer', padding: 0, fontFamily: 'inherit', textDecoration: 'underline' }}>Çıkış yap</button>
      </div>
    );
  }

  const tabBtn = (id, text) => (
    <button type="button" onClick={() => { setTab(id); setErr(''); }} style={{ flex: 1, padding: '11px 0', borderRadius: 10, border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 14.5, fontWeight: 600, background: tab === id ? '#1D130B' : 'transparent', color: tab === id ? '#F5F1E6' : '#55524A' }}>{text}</button>
  );

  return (
    <form onSubmit={submit} style={{ background: '#FFFFFF', border: '1px solid #E8E3D6', borderRadius: 22, padding: '26px 28px 32px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', gap: 6, background: '#F4F1E8', borderRadius: 12, padding: 5 }}>
      ...
      {tab === 'signup' && (
        <>
          ...
          <label style={{ display: 'flex', gap: 10, alignItems: 'flex-start', fontSize: 13, color: '#55524A', lineHeight: 1.6 }}>
      ...
      <button type="submit" disabled={busy} style={{ background: '#1D130B', color: '#F5F1E6', border: 'none', borderRadius: 999, padding: '14px 0', fontSize: 15.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', opacity: busy ? 0.6 : 1 }}>
