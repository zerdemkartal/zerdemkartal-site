  ...
  return (
    <main>
      ...
      <section style={sectionStyle}>
        ...
        <div className="h-module-featured-grid" aria-label="Öne çıkan Hermes çalışma alanları">
          {(moduller.featured || []).map((f, i) => (
            <Reveal key={`${f.title}-${i}`} className="h-r-fill h-module-featured-cell" delay={i * 70}>
              <Spotlight style={cardStyle} className="h-module-card">
                <div className="h-module-media">
                  <img src={f.src} alt={f.alt || `${f.title} ekranı`} loading="lazy" />
                </div>
                <div className="h-module-card-head">
                  <div className="h-module-glyph" aria-hidden="true">{f.glyph}</div>
                  <div style={{ fontFamily: T.serif, fontSize: 20 }} data-he data-path={`home.moduller.featured.${i}.title`}>{f.title}</div>
                </div>
                <p style={{ ...pStyle, fontSize: 14.5, marginTop: 8 }} data-he data-path={`home.moduller.featured.${i}.desc`}>{f.desc}</p>
              </Spotlight>
            </Reveal>
          ))}
        </div>
        <div className="h-module-compact-grid" aria-label="Hermes modülleri">
          {(moduller.cards || []).map((f, i, cards) => {
            ...
            return (
              <Reveal
                key={`${f.title}-${i}`}
                className={`h-r-fill h-module-compact-cell${isClosing ? ' is-closing' : ''}`}
                delay={Math.min((i + 2) * 45, 315)}
              >
                <Spotlight
                  style={isClosing ? { ...cardStyle, padding: '20px 24px' } : cardStyle}
                  className="h-module-card"
                >
                  <div className="h-module-card-head">
                    <div className="h-module-glyph" aria-hidden="true">{f.glyph}</div>
                    <div style={{ fontFamily: T.serif, fontSize: 20 }} data-he data-path={`home.moduller.cards.${i}.title`}>{f.title}</div>
                  </div>
                  <p
                    style={{ ...pStyle, fontSize: 14.5, marginTop: isClosing ? 0 : 8 }}
                    data-he
                    data-path={`home.moduller.cards.${i}.desc`}
                  >
                    {f.desc}
                  </p>
                </Spotlight>
