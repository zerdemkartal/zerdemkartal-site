...
test('Posta UI çöp, toplu işlem, şablon, fatura eki ve güvenli kurulumu birlikte sunuyor', () => {
  ...
  const downloadRoute = fs.readFileSync(path.join(ROOT, 'src/app/api/mail/indirme-baglantisi/route.js'), 'utf8');
  ...
  assert.ok(downloadRoute.includes("requireMailAccess(request, prisma, 'posta.gonder')"));
  assert.ok(downloadRoute.indexOf('requireMailAccess') < downloadRoute.indexOf('request.json()'));
  assert.ok(downloadRoute.includes('createDownloadInvite'));
  assert.ok(downloadRoute.includes('/indir#d=${encodeURIComponent(created.linkToken)}&p=${encodeURIComponent(created.temporaryPassword)}'));
  assert.ok(downloadRoute.includes('createdByRef'));
  assert.ok(!downloadRoute.includes('authorizeLicenseRequest'));
